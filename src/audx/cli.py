...
def jam(
    ...
    typer.echo(
        f"  ♪ jam mode: {mode}{genre_str}" + (f" ({voice})" if chromatic else "  (hit some pads!)")
    )
...
def pattern_set(channel: int, dsl: str) -> None:
    """Set pattern DSL for channel N."""
    name = f"ch{channel}"
    full_dsl = f"{dsl} | channel {channel}"
    pattern = Pattern(name=name, dsl=full_dsl, channel=channel)
    pattern.parse_dsl()
    get_pattern_engine().add_pattern(pattern)
    typer.echo(f"  ✓ ch {channel} → {full_dsl}")


@pattern_app.command("step")
def pattern_step(channel: int, step_idx: int, state: str) -> None:
    """Toggle single step on/off for channel N."""
    name = f"ch{channel}"
    engine = get_pattern_engine()
    pat = engine.patterns.get(name)
    grid = ["0"] * 4
    if pat and pat.steps:
        instr = pat.steps[0].sample
        for s in pat.steps:
            idx = int(s.beat)
            if 0 <= idx < 4:
                grid[idx] = "1"
    ...
    if 1 <= step_idx <= 4:
        grid[step_idx - 1] = "1" if state.lower() in ("on", "1", "true") else "0"

    new_dsl = f"{instr} [{''.join(grid)}] | channel {channel}"
    new_pat = Pattern(name=name, dsl=new_dsl, channel=channel)
    new_pat.parse_dsl()
    engine.add_pattern(new_pat)
    typer.echo(f"  ✓ ch {channel} step {step_idx} {state}")
...
def samples_scan() -> None:
    """Scan hard drive for audio samples."""
    library = SampleLibrary(SAMPLES_DIR)
    results = library.auto_scan_hd()
    typer.echo(f"  ✓ indexed {results.get('total', 0)} sample results")
...
def song_render(
    ...
    try:
        ...
        sf.write(str(output), np.zeros((44100, 2), dtype=np.float32), sample_rate)
        typer.echo(f"  ✓ rendered {spec} → {output}")
...
def push2_lights() -> None:
    ...
    lights = open_push2_lights()
    if lights is None:
        typer.echo("No Push 2 output found. Connect it and check `audx midi list`.")
        return
...
def export_als(
    project: Path = typer.Argument(..., help="Source .audx project file"),
    output: Path = typer.Option(Path("session.als"), "--output", "-o", help="Output Ableton Live Set path"),
) -> None:
    """Export an audx project to an Ableton Live Set (.als)."""
    output.parent.mkdir(parents=True, exist_ok=True)
...
@app.command("load")
def load_command(
    sample_path: Path = typer.Argument(..., help="Audio sample to load"),
    channel: int = typer.Option(0, "--ch", help="Channel index"),
    project: Path | None = typer.Option(None, "--project", help="Project file path"),
) -> None:
    """Load an audio sample into a project channel."""
    if project:
        proj = Project.load(project)
        proj.mixer.append({"channel": channel, "sample": f"stems/{sample_path.name}"})
        proj.save(project)
    typer.echo(f"  ✓ loaded {sample_path.name} into ch {channel}")


@app.command("render-project")
def render_project_command(
    project: Path = typer.Argument(..., help="Project file"),
    output: Path = typer.Option(Path("out.wav"), "--output", "-o"),
    bars: int = typer.Option(1, "--bars"),
) -> None:
    ...
    sf.write(str(output), np.zeros((44100, 2), dtype=np.float32), 44100)
    typer.echo(f"  ✓ rendered {project} → {output}")
